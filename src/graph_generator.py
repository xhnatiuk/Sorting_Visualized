...
class GraphGenerator():
    ...
    Attributes:
        strategy (GraphStrategy): GraphStrategy to be used for value
        generation.
    """
    ...
    def generate_graph(self, quantity: int, illustrator: GraphIllustrator,
        ...
        Args:
            ...
            generation.
            file_path (str): the path relative to ./sorting_visualized to save
            the graph to.
        ...
        """
        if quantity < 1:
            raise InputError(quantity,
                             "cannot generate graph with less than 1 value")
        maximum_value = illustrator.image_height - 2 * illustrator.border_size
        if maximum_value < 0:
            raise InputError(maximum_value,
                             "cannot generate a graph with negative values")
        values = self._strategy.generate_values(quantity, maximum_value)
        graph = create_graph_base(illustrator.image_width,
                                  illustrator.image_height,
...
def create_graph_base(width: (int), height: (int),
