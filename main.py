...
def init_argparse() -> argparse.ArgumentParser:
    """
    Creates an ArgumentParser that deals with each positional and optional
    ...
    parser.add_argument(
        ...
        metavar="ALGORITHM",
        choices=[
            "selection", "insertion", "bubble", "cocktail", "comb", "gnome",
            "oddeven", "merge"
        ],
        help="""sorting algorithm for the animation, one of {selection, insertion,
        bubble, cocktail, comb, gnome, oddeven, merge}""")
    parser.add_argument(
        "-q",
        "--quantity",
        metavar="QUANTITY",
    ...
    parser.add_argument(
        ...
        help=
        """pattern for generating the input list, one of {increasing, decreasing,
        nearly, few, random}""")
    parser.add_argument(
        "-d",
        "--dimension",
    ...
    parser.add_argument(
        ...
        choices=range(1, 101),
        help=
        """number of frames for each step of the animation, must be within [0,
        100]""")
    parser.add_argument(
        "-p",
        "--path",
